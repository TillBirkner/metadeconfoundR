...
BuildConfounderMap <- function(metaDeconfOutput,
  ...
  if (is(metaDeconfOutput, "list")) {
    long_out <- reshape2::melt(metaDeconfOutput$Ps, 
  ...
  if(length(featureColor) == 1 | length(featureColor) == length(unique(long_out$feature))) {
    if(is.null(names(featureColor)) & length(featureColor) == length(unique(long_out$feature))) {
  ...
  } else stop("featureColor should either have length == 1 or the same length as unique features available in metaDeconfOutput!")
  ...
  if (!is.null(featureNames)) {
    ...
    long_out$feature <- map[as.vector(effectSize$feature)]
    
    
    long_out$feature <- factor(as.factor(long_out$feature),
                                      levels = map[levels(long_out$feature)])
  }
  ...
  if (!is.null(metaVariableNames)) {
    ...
    long_out$metaVariable <- map[as.vector(effectSize$metaVariable)]
    
    long_out$metaVariable <- factor(as.factor(effectSize$metaVariable),
  ...
  plot_list <- lapply(unique(long_out$feature), function(x){
    ...
    if(nrow(tmp %>% dplyr::filter(grepl("C:", status))) >= 1){
    ...
    vertices  <-  data.frame(name = unique(c(as.character(edges$from), as.character(edges$to))), 
                             value = 1)
    vertices <- vertices %>%dplyr::mutate(group = edges$from[ match(vertices$name, edges$to) ],
                                          ...
                                          stroke = sapply(tmp[match(vertices$name, tmp$metaVariable),]$status, function(x){
                                            if (is.na(x)) 0.5
                                            else if (!x %in% notTrusted & !grepl("C:", x)) 1.25
                                            else 0.5
                                          }),
                                          id = NA
                      )
    ...
    message(paste0("Plotting ConfounderMap for feature: ",x))
    
    # generate plot
    graph_layout_original <- create_layout(graph = graph, layout = "dendrogram", circular = TRUE)
    # order the nodes properly
    graph_layout <- graph_layout_original %>% dplyr::filter(leaf == T) %>% dplyr::mutate(y_old = y)
    graph_layout$section <- sapply(graph_layout$x,  function(x) ifelse(sign(x) == 1, 1, 2))
    ...
    if(nrow(tmp %>% dplyr::filter(grepl("C:", status))) >= 1) {
      plot <- ggraph(graph = graph_layout_new) + 
        geom_conn_bundle(data = get_con(from = from, to = to), alpha=0.9, aes(colour = ..index..) , width=0.5,tension = 0.5) +
        scale_edge_colour_gradientn(name = "confounding direction",
                                    colors = color_graph, 
                                    values = names(color_graph),
                                    breaks = c(0,1),
                                    labels = c("confounding", "confounded")) +
        geom_node_point(aes(filter = leaf, x = x*1.05, y=y*1.05, fill = effectSize, stroke = stroke), 
                        shape = 21, color = "black", size = 4) +
        scale_fill_gradient2(low = d_col[1], mid = d_col[2], high = d_col[3],
                             n.breaks = 5,
                             na.value = "white",
                             name = "effect size") +
        geom_node_text(aes(x = x*1.15, y=y*1.15, filter = leaf,  label=name, angle = angle, hjust=hjust), size=3, alpha=1) +
        ggtitle(x) +
        theme_void() +
        theme(
        ...
        continuous_scale("stroke", "confounding status", 
                         name = "confounding status",
                         palette = function(x){scales::rescale(x, c(0.5, 1.25))},
                         breaks = c(0.5, 1.25),
