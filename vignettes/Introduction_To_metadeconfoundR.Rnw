...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\subsection{\Rcode{Metadeconfound()}}
		...
		In addition to collecting p-values for all computed tests, effect size is measured if possible. In case of binary data, Cliff's Delta is computed. For continuous data the ``estimate" component from \Rcode{cor.test()} is used. Since there is no suitable effect size metric for categorical data with more than 2 levels, no value is reported here. It is recommended to introduce binary pseudo-variables for each level of the categorical metadata to partially circumvent this drawback.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\subsection{\Rcode{Metadeconfound()}}
		...
		metaMat = metaMatMetformin,
		nnodes = 4,
		randomVar = list("+ (1|Dataset)", c("Dataset"))
		)
		# randomVar = list("+ (1|Dataset) + (1|otherRandEffect)", 
			# c("Dataset", "otherRandEffect")
		@
		...
		Output can be returned either as a list of wide format data.frames (default) or as a single long format data.frame. In both cases raw p-values (Ps), multiple testing corrected p-values (Qs), corresponding effect size (Ds), and confounding status (status) are reported for each possible combination of a feature to a meta-variable. 
		
	\subsection{\Rcode{BuildHeatmap()}}
		
		\subsubsection{input}
				
			\textbf{Minimal input} consists only of an output object from the main \Rcode{Metadeconfound()} function either in long or wide format. This will result in a heatmap similar to \textit{Fig.~\ref{fig:defaultHeatmap}}.
